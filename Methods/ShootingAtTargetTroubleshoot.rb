def ShootingAtTargetTroubleshoot(unit, gear_hash, target, range, moved)
		...
		model.getGear.each do |weapon|
			if gear_hash[weapon]
				...
				gear_hash[weapon].getFiretypes.each do |firetype|
					...
					puts "With a BS of #{bs} and a modifier of #{modifier}, this firetype lands #{hits} hits"
					...
					puts "With a strength of #{str} and a toughness of #{tough}, this weapon causes #{saves} wounds"
					#calculate probability of not saving
					mod_save = save - ap 
					puts "with a save of #{save} and an ap #{ap} the new save is #{mod_save}"
					if mod_save > invuln
						mod_save = invuln
						puts "but the invuln of #{invuln} is higher so that is used instead"
					end
					if mod_save > 7.0
						prob_unsave = 1.0
					else
					...
					puts "With #{saves} wounds and a save of #{save} this model fails #{felt_wounds} saves"
					...
					puts "With a damage of #{d} and #{felt_wounds} failed saves, it does #{dmg_caused} dmg"
